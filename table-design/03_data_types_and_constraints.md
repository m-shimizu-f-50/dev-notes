...
> `UNSIGNED` を付けると正の数だけになり最大値が2倍になる。IDには `UNSIGNED` を付けるのが一般的。
...
| 型 | 説明 | 使いどころ |
|----|------|-----------|
| `CHAR(N)` | 固定長文字列（常にN文字埋められる） | 電話番号・郵便番号など固定長のもの |
...
**TEXT か VARCHAR か：**
- インデックスを貼りたい → `VARCHAR`（TEXTにはデフォルトでインデックスを貼れない）
- WHERE句で使わない長文 → `TEXT`
- 255文字以内 → `VARCHAR`
...
- NULL を禁止する
- **基本的にすべてのカラムに NOT NULL を付けることを推奨**
- NULL は「値が不明」を意味し、予期せぬバグの温床になる
